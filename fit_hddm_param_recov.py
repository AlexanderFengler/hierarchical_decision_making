...
if __name__ == '__main__':
    ...
    CLI.add_argument("--data_path",
                    type = str,
                    default = "data/observed_data_hddm_ready.csv")
    ...
    CLI.add_argument("--out_folder",
                    type = str,
                    default = "data/param_recov/")
    CLI.add_argument("--synthetic_data",
                    type = int,
                    default = 0)
    ...
    # Read in data:
    observed_data = hddm.load_csv(args.data_path)
    ...
    if args.synthetic_data == 0:
        data = observed_data
    else:
    # AF-TODO: Set group_only argument in case we do not want to fit a model that has is_group = True
        data, full_parameter_dict = hddm.simulators.hddm_dataset_generators.simulator_h_c(data = observed_data.copy(),
                                                                                        model = args.model,
                                                                                        p_outlier = 0.00,
                                                                                        conditions = None,
    ...
    if args.synthetic_data == 1:
        ...
        pickle.dump(save_data, open(args.out_folder + args.model + '/' + \
                                        'synth_data_{}_observed_task_{}_coh_{}_group_{}_uuid_{}.pickle'.format(str(args.model),
    ...
    for chain in range(args.nchains):
        ...
        hddm_model_.sample(args.nmcmc, 
                           ...
                           dbname = args.out_folder + args.model + '/' + \
                                    'db_{}_observed_task_{}_coh_{}_group_{}_chain_{}_uuid_{}.db'.format(str(args.model),
                                                                                                     str(args.dep_on_task),
                                                                                                     str(args.dep_on_coh), 
                                                                                                     str(args.is_group_model),
                                                                                                     str(chain),
        ...
        hddm_model_.save(args.out_folder + args.model + '/' + \
                        'model_{}_observed_task_{}_coh_{}_group_{}_chain_{}_uuid_{}.pickle'.format(str(args.model),
                                                                                                str(args.dep_on_task),
                                                                                                str(args.dep_on_coh),
                                                                                                str(args.is_group_model),
                                                                                                str(chain),
